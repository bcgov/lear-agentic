...
export function classifyResidual(logText, patterns) {
  for (const p of patterns || []) {
    if (new RegExp(p, "i").test(logText)) return p;
  }
  return null;
}
...
async function main() {
  ...
  const residual = classifyResidual(logText, cfg.ci_diagnose.known_residuals);
  if (residual) {
    console.log(`Known residual matched (${residual}) — posting short note only.`);
  }
  ...
  if (residual) {
    source = "known-residual";
  } else {
    const findings = analyze(logText);
    narrative = heuristicNarrative(findings);
  ...
  const bodyParts = [
    `### SDLC CI diagnose (${project}) · \`${source}\``,
    "",
    `| | |`,
    `| --- | --- |`,
  ...
  if (residual) {
    bodyParts.push(
      `> **Known residual** (\`${residual}\`) — not a new failure. Fix or remove the underlying job; do not keep merging over it.`,
      "",
    );
  }
  ...
  bodyParts.push(
    ...
    "</details>",
    "",
    `_SDLC · mode=${mode} · source=${source}_`,
  );
  ...
  if (!commented) {
    const title = `SDLC: CI failed — ${workflowName || "workflow"} (#${runId})`;
