 ...
 *
 * Default: refuse to overwrite an existing evidence file (prevents wiping prior slices).
 * Append a new section:
 *   node evidence.mjs --root . --finding AUTH-001 --title "Enable PKCE" --append
 * Force overwrite (rare):
 *   node evidence.mjs --root . --force
 */
...
const cfg = tryLoadConfig(root);
const out = path.resolve(root, arg("out", cfg.paths.evidence));
const append = hasFlag("append");
const force = hasFlag("force");
const finding = arg("finding", "");
const titleArg = arg("title", "");
const template = readFileSync(path.join(__dirname, "templates/EVIDENCE.md.template"), "utf8");
...
const filled = template
  ...
  .replaceAll(
    "{{SPEC_REFS}}",
    features.length ? features.map((f) => `spec/features/${f}`).join(", ") : "spec/spec.md",
  )
