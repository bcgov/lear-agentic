...
export function llmConfigFrom(cfg) {
  const llm = cfg.llm || cfg.agent?.llm || {};
  const keyEnv = llm.api_key_env || "SDLC_LLM_API_KEY";
  const apiKey =
    process.env[keyEnv] ||
  ...
  let apiStyle = (
    llm.api_style ||
    process.env.SDLC_LLM_API_STYLE ||
    process.env.TIER1_LLM_API_STYLE ||
    "openai"
  ).toLowerCase();
  if (apiStyle === "azure" && /azure-api\.net/i.test(baseUrl)) apiStyle = "azure-apim";

  const apiVersion =
    llm.api_version ||
    process.env.SDLC_LLM_API_VERSION ||
    process.env.TIER1_LLM_API_VERSION ||
    process.env.AZURE_OPENAI_API_VERSION ||
    "2025-04-01-preview";

  return { apiKey, baseUrl, model, apiStyle, apiVersion, temperature: llm.temperature ?? 0.2 };
}
...
export async function chatCompletion(cfg, system, user, opts = {}) {
  const { apiKey, baseUrl, model, apiStyle, apiVersion, temperature } = llmConfigFrom(cfg);
  if (!apiKey) {
