...
"""Host-key verification default for sftp-gazette (CONFIG-004)."""
...
def _base_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv('SFTP_ARCHIVE_DIRECTORY', str(tmp_path) + '/')
...
def test_verify_on_missing_host_key_fails_closed(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError, match='SFTP_HOST_KEY is required'):
        SFTPService._connect('drive.qp.gov.bc.ca')
